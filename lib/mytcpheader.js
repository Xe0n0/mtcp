...
DataBuffer.prototype.feed = function(data) {
    ...
    //this.ondata(a.getPackage(data));return;

    if (this.dataleft) {
        // var package=a.getPackage(this.dataleft);

        //console.log(package.sid+" concat "+this.dataleft.length+" with "+data.length+" package.len="+(package.len+HEADER_LENGTH)+" now "+(data.length+this.dataleft.length));
